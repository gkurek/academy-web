...
| K-32 | Sticky `FactsBox` + pasek zamykający                 | Oba                                | Kawałek 5 | **Odrzucono 2026-09-19** — obie opcje (sticky + `ClosingCta`); layout bez zmian. **Świadoma decyzja (D-1, 2026-09-19):** brak CTA po scrollu — zapis tylko w `FactsBox` w nagłówku; sekcja „Jak się zapisać" bez przycisku; znany gap akceptowany. Sticky + opcjonalnie `ClosingCta` → etap 10 (K-37) |
...
| K-35 | CTA „Zapisy" w nagłówku desktop                      | Nie w 04b                          | —         | **Przyjęto 2026-09-19** — poza 04b; backlog etap 10 |
...
**Decyzja: odrzucono 2026-09-19** — obie opcje rekomendacji (sticky `FactsBox` + `ClosingCta`); layout ofertowy bez zmian. Po prototypie na stagingu: sticky odrzucone; `ClosingCta` wycofane razem ze sticky. **Świadoma decyzja D-1 (2026-09-19):** brak CTA po scrollu — zapis tylko w `FactsBox` w nagłówku strony; sekcja „Jak się zapisać" bez przycisku; znany gap akceptowany. **Sticky + opcjonalnie `ClosingCta` → ponowne rozważenie w etapie 10** (K-37).
...
Szuflada mobilna ma blok CTA, desktop nie. **Rekomendacja: nie w 04b.** Przycisk musiałby wiedzieć, dokąd prowadzić poza okresem naboru; wrócić do tematu w etapie 10, kiedy będą dane z analityki. Wpisać do backlogu.
...
- [x] ~~CTA zapisu dostępne na końcu każdej strony ofertowej i `/wyklady`~~ **N/A** — świadoma decyzja D-1 (2026-09-19): brak CTA po scrollu; zapis tylko w `FactsBox`; powrót do tematu w etapie 10 (K-37)
...
- **Poza zakresem, do backlogu:** CTA w nagłówku desktop (K-35); **sticky `FactsBox` (+ opcjonalnie `ClosingCta`) — ponowne rozważenie w etapie 10** (K-37); **design stopki — dopracowanie w etapie 10** (K-36); wspólna obróbka zdjęć w filarach (przyciemnienie, proporcje) — razem z sesją zdjęciową;
...
| 5 — Przepływ sprzedażowy      | ✅      | K-32 odrzucone (sticky + `ClosingCta` wycofane; K-37 → etap 10). K-33 częściowo: H2 Najbliższe, filary → huby. **Hero opcja A** (`min(920px, 76vh)`, siatka md+; fold nie wymagany) — patrz K-33 pkt 1. K-34 odłożone. Layout ofertowy przywrócony (boks tylko w nagłówku). Build/lint OK.                                                                                                                                                                                                          |
