...
| 5   | Galeria ikon (+ korekty 05b)                   | `docs/plans/05-galeria.md`, `docs/plans/05b-review-fixes.md` | 🟠 w implementacji (05: 4/4, 05b: 6/6 — czeka na OK) | —          |
...
| K-05 | Filtry galerii: query string vs stan               | 5        | **Query string** (`?temat=<slug-tagu>`), bez przeładowania; domyślnie wszystkie tematy. **`?autor=` wycofane w 05b (K-43)** — podział EJK / uczniowie jest sztywny (sekcje, K-41), do „tylko prace uczniów” linkuje hash `#uczniowie` — patrz `docs/plans/05-galeria.md`, `docs/plans/05b-review-fixes.md` | 2026-09-19 |
...
| K-40 | Układ siatki galerii (05b; zmienia K-31 dla galerii) | 5b | **Opcja A „półka”** — stałe kolumny 4 / 3 / 2, ramka z proporcją zdjęcia przyciętą do 5:8…4:3, dół rzędu na wspólnej linii (subgrid), podpisy w jednym rzędzie; kolejność czytania rzędami. Na 21 realnych pracach A zostaje (B — wyrównane rzędy — odrzucone, choć rzędy z bardzo wysoką i poziomą pracą mają ~200 px pustki nad niższym obrazem). „Wybrane ikony” na home zostają przy K-31 — patrz `docs/plans/05b-review-fixes.md` | 2026-09-19 |
| K-41 | Kolejność: EJK → uczniowie (05b) | 5b | **Dwie sztywne sekcje** z H2 i licznikiem (`#ejk`, `#uczniowie`), kotwice w nagłówku; kolejność w sekcji = kolejność w `icons.json`; sekcja pusta pod filtrem tematu znika razem z kotwicą. Doprecyzowuje D-02 | 2026-09-19 |
| K-42 | Podpisy i atrybucja autorów (05b) | 5b | **Trzy poziomy:** siatka — sam tytuł; sekcja uczniów — wstęp + lista nazwisk generowana z danych; lightbox — pełny autor lub „Praca z warsztatów Akademii”. `authorName?` opcjonalne w `IconWork`; wymiar tylko w lightboxie i jako „do weryfikacji” | 2026-09-19 |
| K-43 | Logika filtrów (05b) | 5b | **Jeden filtr — temat:** „Wszystkie”, wybór jednokrotny, `aria-current`, grupa z `aria-label`. **Filtr autora i `?autor=` wycofane.** Taksonomia: Chrystus · Matka Boża · Aniołowie · Święci · Sceny i święta (stała lista, kolejność chipów z niej); walidacja przy buildzie (każda praca ≥ 1 tag, każdy temat ≥ 1 praca, tag spoza taksonomii); nieznany `?temat=` usuwany z URL | 2026-09-19 |
| K-44 | Nagłówek galerii (05b) | 5b | **Opcja A** — od 1024 px dwie kolumny (H1 + lead / filtr tematu + kotwice), mobile: poziomy pasek chipów z uciętym ostatnim chipem. Świadome odstępstwo od K-23 (filtr to nie nawigacja). Przy 6 chipach 2 rzędy na desktopie | 2026-09-19 |
...
| K-46 | Lead galerii i zajawka zamówienia (05b) | 5b | Lead: „… Ikonę na konkretne wezwanie można zamówić w pracowni.” (**do akceptacji EJK**). Zajawka **bez zdjęcia** do sesji zdjęciowej; CTA jako przycisk | 2026-09-19 |
| K-47 | Treść `sample` galerii (05b) | 5b | **Opcja A** — 21 prac z WP (11 EJK + 10 uczniów, oryginały), tytuły zgodne ze zdjęciami. Wymiary z podpisów WP zostają w danych, UI pokazuje „Wymiary: do weryfikacji” (bez nowego pola). Z makiety wypadły „Św. Antoni” i „Przemienienie” (brak w galerii WP); dawny „Mandylion” to Chrystus Pantokrator, „Matka Boża Znaku” to Krzew Gorejący | 2026-09-19 |
...
| Wymiary ikon `[z podpisu WP]` (`size`) | `content/icons.json`                               | 2, 5 (05b/5: 20 z 21 prac ma `size`) | potwierdzenie przez EJK; do tego czasu UI pokazuje „Wymiary: do weryfikacji” (`pl.gallery.lightbox.sizeUnverified`), liczb nie wyświetla | ⬜  |
| Zestaw sample galerii — 21 prac z WP (11 EJK + 10 uczniów), oryginały w `public/media/sample/icons/`, kolejność ręczna | `content/icons.json`, `public/media/sample/icons/*` | 5 (05b/5, zastępuje 14 wpisów na 4 zdjęciach) | migracja WP (pod-etap 8) — to wyciąg z migracji, nie import ponowny | ⬜ |
| Nazwiska uczniów w dopełniaczu jak na WP („Izabeli Jeleniewskiej”); 4 prace uczniów bez nazwiska (fallback „Praca z warsztatów Akademii”) | `content/icons.json` (`authorName`) | 5 (05b/5) | mianownik, poprawiony przez właściciela; zgoda Akademii na publikację nazwisk | ⬜ |
| Wstęp do sekcji uczniów `[do uzupełnienia]`, etykieta „Autorzy prac:”, etykiety kotwic („Ikony EJK”), lead galerii (K-46) | `src/i18n/pl.ts` (`gallery.sections`, `gallery.lead`) | 5 (05b/3, 05b/5) | tekst od Akademii / akceptacja EJK | ⬜ |
| Zajawka „Ikony na zamówienie” bez zdjęcia (K-46) — do sesji zdjęciowej; `public/media/sample/gallery-order-teaser.jpg` nieużywany | `src/components/gallery/GalleryOrderTeaser.tsx` | 5 (Kawałek 1), zmiana w 05b/5 | zdjęcie z sesji zdjęciowej | ⬜ |
...
| „Wybrane ikony” na stronie głównej i „Przykłady realizacji” w `/ikony/na-zamowienie` — 4 prace z 21 (Krzew Gorejący, Pantokrator, Archanioł Michał, Trójca Święta) | `FEATURED_ICON_SLUGS` w `src/content/icons.ts`, `exampleSlugs` w `content/offers/zamowienie.mdx` | 2, zmiana w 05b/5 | wybór redakcyjny EJK z prac galerii | ⬜ |
