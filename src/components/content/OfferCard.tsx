  ...
  return (
    <article className="bg-surface-tile flex flex-col">
      ...
      <div className="flex flex-1 flex-col px-offer-card-x pt-offer-card-y pb-offer-card-pb">
        <p className="font-serif text-size-nav text-accent mb-space-3">{eyebrow}</p>
        <h2 className="font-serif text-size-offer-card-title font-medium leading-heading text-text-h2 mb-space-4">
          {title}
        </h2>
        <p className="text-size-body leading-body text-text-secondary mb-space-4">{excerpt}</p>
        <ul className="mb-space-5 list-none text-size-body leading-loose text-text-secondary">
          {bullets.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
